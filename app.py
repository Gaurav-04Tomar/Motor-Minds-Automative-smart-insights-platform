...
@st.cache_resource
def train_predictive_maintenance_model(df, selected_features, max_depth, min_samples_split, min_samples_leaf, learning_rate, epochs):
    # Prepare data
...
def run_spare_parts_forecasting():
    ...
    if spare_part_input:
        ...
        plot_forecast(weekly_data[:-test_split], weekly_data[-test_split:], hw_predictions, title=f'Holt-Winters Forecast for {spare_part_input}')
        ...
        plot_forecast(weekly_data[:-test_split], weekly_data[-test_split:], sarima_predictions, title=f'SARIMA Forecast for {spare_part_input}')
...
def forecast_holt_winters(weekly_data, test_split=16, seasonal_periods=26):
    ...
    model = ExponentialSmoothing(train_data['spare_part'], trend='mul', seasonal='add', seasonal_periods=seasonal_periods).fit()
